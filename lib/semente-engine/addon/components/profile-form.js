...
export default Ember.Component.extend({
  ...
  platAnos: Ember.computed(function() {
    var platAnosPortal = this.get('store').peekAll('instituicao-plataforma-ano-sistema').filterBy('sistema.idx', 1).mapBy('plataformaAno').sortBy('idx');
    return platAnosPortal;
  ...
  segmentos: Ember.computed('pessoa', function () {
    var segmentosPortal = this.get('store').peekAll('instituicao-plataforma-ano-sistema').filterBy('sistema.idx', 1).mapBy('plataformaAno').mapBy('segmento');
    let segmentosProtalFiltered = segmentosPortal.filter((value, index) => segmentosPortal.mapBy('id').indexOf(value.get('id')) == index);
    return segmentosProtalFiltered.sortBy('idx');
  }),
  ...
  afterRenderInformation: Ember.computed('model',function(){
    if(this.get('pessoa') != null){
      if(this.get('pessoa').get('role') == 'instrutor'){
        this.get('pessoa').get('plataformaTurmas').forEach(pt => {
          if(document.getElementById('plat_turma' + pt.get('id'))){
  ...
  actions: {
    ...
    refreshSelectedFunction(selectedFunction) {
      let pessoa = this.get('pessoa');
      pessoa.set('function', selectedFunction);
    },
