...
export default Controller.extend({
  ...
  selected_segmento_id: Ember.computed('model', function () {
    ...
    if(firstObject != null){
      if(firstObject.get('idx') == 4) this.set('isMedio', true);
      if(firstObject.get('idx') == 6) this.set('isAdulto', true);
  ...
  plataformaAnos: Ember.computed('model', function () {
    var platAnosPortal = this.get('store').peekAll('instituicao-plataforma-ano-sistema').filterBy('sistema.idx', 1).mapBy('plataformaAno');
    let plataformaAnos = platAnosPortal.filterBy('segmento.idx', this.get('selectedSegmento').get('idx'));
  ...
  acompanhamentoPessoas: Ember.computed('model', function () {
    return this.get('model').get('acompanhamentoPessoasPlataforma');
  }),
  ...
  acompanhamentoAtividade: Ember.computed('model', function () {
    return this.get('model').get('acompanhamentoAtividadesPlataforma');
  }),
  ...
  pessoasDetalhado: Ember.observer('model',function() {
    ...
    this.get('store').query('pessoa', {
      instituicao_id: this.get('instituicao').get('id'),
  ...
  }).property("selected_ano_id", "selected_turma_id", "search"),
