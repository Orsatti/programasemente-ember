...
export default Controller.extend({
  ...
  queryParams: ["selected_segmento_id", "selected_ano_id", "selected_turma_id", "selected_start_date", "selected_end_date"],
  ...
  actions: {
    ...
    refreshSelectedStartDate(date){
      if(date != null && date != ''){
        this.set('selected_start_date', date);
      }
    },

    refreshSelectedEndDate(date){
      if(date != null && date != ''){
        this.set('selected_end_date', date);
      }
    },
    ...
    downloadFile() {
      let url = this.get('envnmt.host') + '/' + this.get('envnmt.namespace') + '/relatorios/alunos?pessoaId=' + this.get('pessoa').get('id') + '&instituicaoId=' + this.get('model.instituicao').get('id');
      if (this.get('selected_segmento_id') != null) url = url + "&segmentoId=" + this.get('selected_segmento_id');
      if (this.get('selected_ano_id') != null) url = url + "&platAnoId=" + this.get('selected_ano_id');
      if (this.get('selected_turma_id') != null) url = url + "&platTurmaId=" + this.get('selected_turma_id');
      window.location.href = url;
      $("#btn-download").attr("disabled", true);
      $("#downloading").addClass("alert--is-show");
      setTimeout(function(){ 
        $("#btn-download").attr("disabled", false);
        $("#downloading").removeClass("alert--is-show"); 
      }, 4000);
    },
  }
