...
export default Controller.extend({
  ...
  oneInst: Ember.computed('model', 'role', function () {
    ...
    if (!isArray(inst)) {
      ...
      return true;
    }else if (inst.content.length == 1) {
      Ember.run.once(function () {
        that.set('inst_selected', false);
        that.setSelected(inst.content[0].id);
        that.transitionToRoute('aulas.acompanhamento', {
          queryParams: {
            instituicao_id: inst.content[0].id,
            page: 1,
          }
        });
        that.set('instView', true);
  ...
  async setSelected(inst) {
    ...
    let inst_selected = await this.get('store').findRecord('instituicao', inst, {
      ...
      include: 'modulos.atividades, plataforma-turmas.plataforma-ano, plataforma-anos.segmento, acompanhamentos-atividade-instituicao', reload: true
    });
