...
export default Controller.extend({
  ...
  queryParams: ["page", "perPage", "instituicao_id", "ordem", "str_search", "anos", "pre", "tur", "platAno", "shouldReviewProfile"],
  ...
  curso_selected: Ember.computed('model', function(){
    ...
    this.set('anos', mods.get('firstObject').get('id'));
    return mods.get('firstObject');
  }),

  selectedModuloName: 'Primeiro Ano',
  selectedTurmaName: 'Todas as turmas',

  hasChildren: Ember.computed('model', function () {
    let inst = this.get('inst_selected');
    if (inst.get('instituicaoFilhas').content.length > 1) {
      return true;
    }
    return false;
  }),
  ...
  acompanhamentosatividades:Ember.computed('model', function () {
  ...
  actions: {
    ...
    filterano() {
      let select = document.getElementById('mod_selector_det');
      let mod = select.options[select.selectedIndex].value;
      let modulo = this.get('store').peekRecord('modulo', mod);
      this.set('selectedModuloName', modulo.get('name'));      
      this.set('anos', mod);
    },
    filterTurma() {
      let select = document.getElementById('turma_selector_det');
      ...
      if (turma != 0) {
        let turmaModel = this.get('store').peekRecord('turma', turma);
        this.set('selectedTurmaName', turmaModel.get('name'));
      } else {
        this.set('selectedTurmaName', 'Todas as turmas');
      }
      this.set('tur', turma);
    },
    refreshContent(c) {
