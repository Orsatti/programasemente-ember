...
export default Controller.extend({
  ...
  modulo: Ember.computed('model', function() {
    
    if (this.get('pessoa').get('modulos').content.length > 0) {
      return this.get('pessoa').get('modulos').get('firstObject');
    }
    return null
  }),
  ...
  conteudosList:Ember.computed('model', function () {
    return this.get('conteudos');
  }).property('conteudos'),
  ...
  conteudos: Ember.computed('model', function () {
    ...
    if(this.get('model.pessoa').get('role') == 'aluno'){
      conts = conts.filter(c => this.get('model.pessoa').get('plataformaAnos').map(y => y.get('id')).some(x => c.get('plataformaAnos').map(x => x.get('id')).indexOf(x) > -1))
    }
  ...
  actions: {
    ...
    filterCards(){
      this.send('animateCardList');
      let substr = document.getElementById('search').value;
      let conteudos = this.get('conteudos');
      let listFiltered = [];
      conteudos.forEach(conteudo => {
        if(conteudo.get('titulo').indexOf(substr) != -1){
          listFiltered.pushObject(conteudo);
        } else if(conteudo.get('descricao') != null){
          if(conteudo.get('descricao').indexOf(substr) != -1){
            listFiltered.pushObject(conteudo);
          }
        }
      });

      this.set('conteudosList', listFiltered)
    }
  },
});
