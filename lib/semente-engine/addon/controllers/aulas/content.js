...
export default Controller.extend({
  ...
  plataformaTurmas: Ember.computed('model', function(){
    return this.get('store').query('plataforma-turma', {plataformaAnoId: this.get('model.aula').get('plataformaAno').get('id')});
  }),
  TurmasAplicadas: Ember.computed('aula', function(){ 
    let turmasAplicadas = this.get('aula').get('aplicacoes').filterBy('aplicado', true).mapBy('turma');
  ...
  conteudos: Ember.computed('aula', function(){
    ...
    if (['gestor', 'coordenador', 'recursosHumanos'].includes(this.get('pessoa.role'))){
      return conts.filterBy('situacao', true).filterBy('agrupamento.name', this.get('selectedAgrupamento'));
    }
  ...
  avaliacao: Ember.computed('aula', function(){
    let pessoa = this.get('pessoa');
    ...
    sendSuggestion() {
      ...
      let params = JSON.stringify({
        "data": {
          ...
          "id": "0",
          "attributes": {
            "email-origem": "Plataforma@programasemente.com.br",
            "email-destino": "sugestoes@programasemente.com.br",
