...
export default Controller.extend({
  ...
  plataformaAnos: Ember.computed('model', function () {
    var plataformaAnos = this.get('store').peekAll('plataforma-ano').sortBy('idx');
    if(this.get('pessoa').get('role') == 'aluno'){
      plataformaAnos = this.get('pessoa').get('plataformaAnos');
    }
    if(this.get('pessoa').get('role') == 'instrutor'){
      plataformaAnos = this.get('pessoa').get('plataformaTurmas').mapBy('plataformaAno').filter((value, index, array) => array.indexOf(value) === index);
    }
  ...
  aulasFiltradas: Ember.computed('model', function () {
    ...
    if(this.get('pessoa').get('role') != 'admin' && aulas.length > 0){
  ...
  }).property('selectedSegmento', 'selectedAno', 'search', 'ordem', 'selectedCompId', 'selectedSituacao'),
