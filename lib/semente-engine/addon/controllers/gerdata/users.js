...
export default Controller.extend({
  ...
  queryParams: ["page", "perPage", "ordem", "str_search", "selected_segmento_id", "selected_ano_id", "selected_turma_id", "selected_instituicao_filha"],
  ...
  pessoas_sorted: Ember.computed('pagedContent.content', 'sortCol', 'sortAsc', 'reSort', 'onlyEnabled', function () {
    document.getElementById('search_input_pessoas_ger').value = this.get('str_search');
    let people = this.get('pagedContent.content');
    if (people) {
      let peopleFiltered = [];
      people.filter((x) =>{
        if(this.get('onlyEnabled'))
            if(!x.get('enabled') && !x.get('acessoPlataformaS') && !x.get('acessoCs'));
            else peopleFiltered.push(x);
        else if(!x.get('enabled') && !x.get('acessoPlataformaS') && !x.get('acessoCs')) peopleFiltered.push(x);
      });

      let sortCol = this.get('sortCol');
      if (this.get('sortAsc') && this.get('reSort') < 3)
      return peopleFiltered.sortBy(sortCol, 'DimensionName');
      else return peopleFiltered.sortBy(sortCol, 'DimensionName').reverse();
    } else return [];
  }),
...
//   },
  ...
  acompanhamentoSistemas: Ember.computed('model', function () {
    return this.get('model.acompanhamento');
  }),
  ...
  actions: {
    ...
    setEnableds(){
      let checked = document.getElementById('only-enabled').checked;
      this.set('onlyEnabled', checked);
    },
