...
export default Controller.extend({
  ...
  instituicao: Ember.computed('model', function(){
    ...
    let inst = this.get('store').peekRecord('instituicao', this.get('model').otherParams.instituicao_id);
    if(inst.get('instituicaoFilhas').content.length > 0) this.set('instituicaoMae', true);
    return inst;
  }),
