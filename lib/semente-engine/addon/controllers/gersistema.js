...
export default Controller.extend({
  ...
  codigosCadastrosAlunos: Ember.computed('inst_selected', async function () {
  ...
  codigosCadastrosProfessores: Ember.computed('inst_selected', function () {
    //this.get('store').findRecord('instituicao', this.get('inst_selected').get('id'), { include: 'codigos-cadastro' });
    // let codigos = this.get('inst_selected').get('codigosCadastro')
    // let arrayCodigos = codigos.toArray();
    // let codigosProfessores = []

    // arrayCodigos.forEach(function (c) {
    //   if (c.get('typeCadastro') == 'instrutor') {
    //     codigosProfessores.push(c);
    //   }
    // })
    return []
  }),
    ...
    newInstSave() {
      ...
      sistemas.forEach((sis) => {
        if (document.getElementById('new_sis_' + sis.data.idx).checked) {
          ...
          if (sis.data.idx == 1) {
            modulos.forEach((mod) => {
              if (document.getElementById('ckb_new_' + 12).checked && mod.id == 11) {
                selected_modulos.pushObject({
                  "id": mod.id,
                  "type": "modulo"
                });
              } else if (document.getElementById('ckb_new_' + 13).checked && mod.id == 12) {
                selected_modulos.pushObject({
                  "id": mod.id,
                  "type": "modulo"
                });
              } else if (document.getElementById('ckb_new_' + 14).checked && mod.id == 13) {
                selected_modulos.pushObject({
                  "id": mod.id,
                  "type": "modulo"
                });
