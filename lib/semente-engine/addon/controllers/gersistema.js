...
export default Controller.extend({
  ...
  listSegPlatAnos: [],
  plataformaAnos: Ember.computed('pessoa', function () {
    let platAnos = [];
    let segmentos = this.get('segmentos');
    segmentos.forEach(seg => {
      seg.get('plataformaAnos').forEach(pa => [
        platAnos.pushObject(pa)
      ])
    })
    return platAnos;
  }),
  calendario: Ember.computed('model', function () {
    return this.get('inst_selected').get('calendario');
  }),
  calendarios: Ember.computed('model', function () {
    return this.get('store').peekAll('calendario');
  }),

  codigos: Ember.computed('inst_selected', async function () {
    //this.get('store').findRecord('instituicao', this.get('inst_selected').get('id'), { include: 'codigos-cadastro' });
    let instId = parseInt(this.get('inst_selected').get('id'));
    let codigos = await this.get('store').query('codigo-cadastro', {instId: instId});
    let arrayCodigos = codigos.toArray();
    let codigosAlunos = []
    let codigosprofs = []

    for (let i = 0; i < arrayCodigos.length; i++) {
      const element = arrayCodigos[i];
      if (element.get('typeCadastro') == 'aluno') {
        codigosAlunos.push(element);
      }
    }
    for (let i = 0; i < arrayCodigos.length; i++) {
      const element = arrayCodigos[i];
      if (element.get('typeCadastro') == 'instrutor') {
        codigosprofs.push(element);
      }
    }
    this.set('codigosCadastrosAlunos', codigosAlunos);
    this.set('codigosCadastrosProfessores', codigosprofs);
  ...
  oneInst: Ember.computed('model', 'role', function () {
    ...
    let that = this;
    if (!isArray(inst) || inst.get('length') == 1) {
      if (isArray(inst)) inst = inst.get('firstObject');
  ...
  goToStepTwo: false,
  areaToDelete: null,
  selectedInstTabs: Ember.computed('inst_selected', function () {
    let infosLogged = JSON.parse(localStorage.getItem('person_logged'));
    ...
    if ((pessoa.get("role") == "gestor") || (pessoa.get("role") == "recursosHumanos")) {
      return {
        informations: false,
        courses: false,
        areas: false,
  ...
  async setSelected(inst) {
    ...
    let inst_selected = await this.get('store').findRecord('instituicao', inst, {
      include: 'modulos.atividades, plataforma-anos.segmento, sistemas, turmas, plataforma-turmas, areas', reload: false
    });
    ...
    this.set('instTemS', this.get('temS'));
  ...
  redirectToTurmas: Ember.computed(function () {
    ...
    if (pessoa.get('role') == 'secretaria' || pessoa.get('role') == 'gestor'|| pessoa.get('role') == 'recursosHumanos') {
  ...
  actions: {
    ...
    deleteInstituicaoModal: function () {
      document.getElementById('confirm_modal').classList.remove('fadeOutDown');
      document.getElementById('confirm_modal').classList.add('modal--is-show');
    },
    deleteInstituicao(){
      this.get('inst_selected').destroyRecord().then((response) => {
        this.set('timeoutTxt', "instituicao excluída com sucesso");
        this.timeoutAlert('success');

        this.send('closeModal', 'confirm_modal');
        this.send('backToInstList');
      });
    },
    closeModal(modal) {
      document.getElementById(modal).classList.remove('modal--is-show');
    },
    ...
    newInst() {
      ...
      document.getElementById('new_inst_nrsecretaria').value = '';
      document.getElementById('new_inst_nrmarketing').value = '';
      document.getElementById('new_inst_enabled').checked = true;
      document.getElementById('new_inst_isSchool').checked = true;
      document.getElementById('new_inst_termoAceite').checked = true;
      document.getElementById('new_sis_1').checked = true;
      ...
      platAnos.forEach((pa) => {
        document.getElementById('ckb_new_' + pa.id).checked = true;
      })
    ...
    checkboxSPlus(){
      let checkbox = document.getElementById('new_sis_2').checked;
      if(checkbox){
        this.set('newInstTemS', true);
      } else{
        document.getElementById('new_sis_sPlus').checked = false;
        this.set('newInstTemS', false);
      }
    },
    ...
    newInstSave() {
      ...
      let radical = document.getElementById('new_inst_rad').value;
      let enabled = document.getElementById('new_inst_enabled').checked;
      let isSchool = document.getElementById('new_inst_isSchool').checked;
      let termoAceite = document.getElementById('new_inst_termoAceite').checked;
      ...
      let nr_secretaria = document.getElementById('new_inst_nrsecretaria').value;
      let nr_marketing = document.getElementById('new_inst_nrmarketing').value;
      ...
      let sPlusEnabled = false;
      if(this.get('newInstTemS')){
        let sPlus = document.getElementById('new_sis_sPlus').checked;
        sPlusEnabled = sPlus;
      }

      let platAnos = this.get('plataformaAnos');
      let selected_platAnos = [];
      platAnos.forEach((pa) => {
        if (document.getElementById('ckb_new_' + pa.id).checked) {
          selected_platAnos.pushObject(pa);
        }
      });
      ...
      if (errors === 0) {
        let obj_inst = this.get('store').createRecord('instituicao', {
          ...
          "isEscola": isSchool,
          "radical": radical,
          "statusTermoAceite": termoAceite,
          "sPlusEnabled": sPlusEnabled,
          //"image":image,
          "nrgestores": nr_gestores,
          "nralunos": nr_alunos,
          "nrinstrutores": nr_instrutores,
          "nrcoordenadores": nr_coordenadores,
          "nrsecretaria": nr_secretaria,
          "nrmarketing": nr_marketing,
        ...
        if (selected_platAnos.length > 0) {
          selected_platAnos.forEach((pa) => {
            let temp = this.get('plataformaAnos');
            temp.forEach((platAno) => {
              if (platAno.id === pa.id) {
                obj_inst.get('plataformaAnos').pushObject(platAno);
              }
            });
          });
        }
    ...
    backToInstList() {
      ...
      this.set('selectedInstTabs.codes', false);
    ...
    dataChange(param, param2) {
      ...
      if (param == 'btn_save_inst') {
        ...
        if (document.getElementById('inst_isSchool').checked != this.get('inst_selected.isEscola')) changes++;
        if (document.getElementById('inst_termoAceite').checked != this.get('inst_selected.statusTermoAceite')) changes++;
        if (document.getElementById('inst_rad').value != this.get('inst_selected.radical')) changes++;
        ...
        if (document.getElementById('nr_secretaria').value != this.get('inst_selected.nrsecretaria')) changes++;
        if (document.getElementById('nr_marketing').value != this.get('inst_selected.nrmarketing')) changes++;
        ...
        if(this.get('instTemS')) {
          if (document.getElementById('inst_sPlus').checked != this.get('inst_selected.sPlusEnabled')) changes++;
        }
      }
      
      if (document.getElementById('sis_2').checked) {
        this.set('instTemS', true);
      }
      else{
        this.set('instTemS', false);
      }
    ...
    calendarioChanged(selectedCalendarioId) {
      let calendario = this.get('inst_selected').get('calendario');
      let selectCalendario = this.get('store').peekRecord('calendario', selectedCalendarioId);
      if(calendario.get('id') != selectedCalendarioId){
        this.set('calendario', selectCalendario);
        document.getElementById('calendario_' + calendario.get('id')).checked = false;
        document.getElementById('calendario_' + selectCalendario.get('id')).checked = true;
        document.getElementById('btn_save_inst').style.display = 'inherit';
      }
      else{
        document.getElementById('calendario_' + this.get('calendario').get('id')).checked = false;
        this.set('calendario', selectCalendario);
        document.getElementById('calendario_' + selectCalendario.get('id')).checked = true;
    ...
    saveInformations() {
      ...
      let radical = document.getElementById('inst_rad').value;
      ...
      let secretaria = document.getElementById('nr_secretaria').value;
      let marketing = document.getElementById('nr_marketing').value;
      let ena = document.getElementById('inst_hab').checked;
      let is = document.getElementById('inst_isSchool').checked;
      let termoAceite = document.getElementById('inst_termoAceite').checked;
      ...
      if (errors === 0) {
        ...
        if(this.get('instTemS')){
          let sPlus = document.getElementById('inst_sPlus').checked;
          inst.set('sPlusEnabled', sPlus);
        }

        inst.set('name', name);
        inst.set('radical', radical);
        inst.set('nrgestores', gest);
        inst.set('nrinstrutores', instr);
        inst.set('nralunos', aluno);
        inst.set('nrcoordenadores', coord);
        inst.set('nrsecretaria', secretaria);
        inst.set('nrmarketing', marketing);
        inst.set('enabled', ena);
        inst.set('isEscola', is);
        inst.set('statusTermoAceite', termoAceite);
        ...
        inst.set('calendario', this.get('calendario'));
    ...
    addArea() {
      ...
      if (name === null || name === '' || name === ' ') {
        ...
        setTimeout(() => {
          ...
          targetInput.focus();
          this.set('timeoutTxt', "Insira o nome da turma");
          this.timeoutAlert('error');
        }, 100);
      ...
      if (errors == 0 && !ano) {
        let targetInput = document.getElementById("define_year");
        targetInput.closest('.j-areaFocus').classList.remove('blink');

        setTimeout(() => {
          targetInput.closest('.j-areaFocus').classList.add('blink');
          targetInput.focus();
          this.set('timeoutTxt', "Selecione o ano");
          this.timeoutAlert('error');
        }, 100);
      ...
      if (errors === 0) {
        ...
        turma.save().then(() => {
          ...
          if (reason.errors[0].status == "400") {
            ...
            this.set('timeoutTxt', reason.errors[0].title);
            this.timeoutAlert('error');
          }
          else if (reason.errors[0].status == "409") {
    ...
    downloadCodigos() {
      ...
      window.location.href = this.get('envnmt.host') + '/' + this.get('envnmt.namespace') + '/instituicoes/downloadCodigos?pessoaId=' + header_inst + '&instituicaoId=' + inst.id;
    },
