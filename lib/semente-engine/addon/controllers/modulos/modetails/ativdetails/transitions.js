...
isEja: Ember.computed('model', function(){

  
  let str = localStorage.getItem('person_logged');
  if (str) {
      str = JSON.parse(str);
      let person = this.get('store').peekRecord('pessoa', str.id);
      return person.get('isEja')
    }
  ...
  getOut() {
      ...
      if (ultimaObrigatoria) {
        ...
        if (ultimaDaAula) {
          if (dataNextAtividade < diaAtual) {
            ...
            this.transitionToRoute('modulos.modlist', this.get('model').get('atividade').get('modulo').get('id'))
          }

        } else {
          //this.transitionToRoute('modulos');
          this.transitionToRoute('modulos.modlist', atividade.get('modulo').get('id'));
        }
      } else {
        if (proximaSecao) {
          // tem próxima seção
          if (proximaQuiz) {
            ...
            this.transitionToRoute('modulos.modetails.ativdetails.secdetails', proximaSecao);
          }
        }
      }
    }
    /*
        if (path.search('transition') > 1) {
          ...
          else if (tgt) this.transitionToRoute('modulos.modetails.ativdetails.secdetails', tgt);
          else if (tgt_ativ) {
            if (this.get('next_section_quiz')) this.transitionToRoute('modulos.modetails.ativdetails.secdetails', tgt_ativ.get('modulo').get('id'), tgt_ativ.get('id'), this.get('next_ativsec_id'), {
              queryParams: {
                questao: 1
              }
            });
            else this.transitionToRoute('modulos.modetails.ativdetails.secdetails', tgt_ativ.get('modulo').get('id'), tgt_ativ.get('id'), this.get('next_ativsec_id'));
          }
        }*/
  },
  proximaSecao: Ember.observer("model", "appController.transited", function () {
    ...
    if (next_atividade) {
      ...
      let stringdiaAtividade = next_atividade.data.dia;
      let dataNextAtividade = new Date(stringdiaAtividade);
      let ignoraCalendarioMedio = this.get('instituicao').get('ignoraCalendarioMedio');
      
            
      if (ignoraCalendarioMedio) {
        this.set('next_atividade_liberada', true);
      } else if (dataNextAtividade < diaAtual) {
        this.set('next_atividade_liberada', true);
      } else {
        this.set('next_atividade_liberada', false);
      }
  ...
  actions: {
    reverVideo() {
      ...
      this.transitionToRoute('modulos.modetails.ativdetails.secdetails', model, {
        queryParams: {
          videoini: true
        }
      });
    ...
    goNext(param1, param2, param3) {
      ...
      this.set('counting', false);
      if (param3 === 'section') {
        if (param2) this.transitionToRoute('modulos.modetails.ativdetails.secdetails',
        ...
        else this.transitionToRoute('modulos.modetails.ativdetails.secdetails',
          model.get('atividade').get('modulo').get('id'),
          model.get('atividade').get('id'),
          param1);
      } else if (param3 === 'atividade') {
        this.transitionToRoute('modulos.modetails.ativdetails.secdetails', model.get('atividade').get('modulo').get('id'), param1, param2);
      } else if (param3 == 'modulo') {
        //this.transitionToRoute('modulos');
        this.transitionToRoute('modulos.modlist', model.get('modulo').get('id'));
      }
    },
    goBack() {
      document.getElementById('course-nav').style.zIndex = "2";
      ...
      clearInterval(this.get('active_counter'));
      this.set('counting', false);
      this.transitionToRoute('modulos.modlist', model.get('atividade').get('modulo').get('id'))
    }
  }
});
