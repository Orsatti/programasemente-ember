  ...
  model() {
    ...
    if (role === 'admin') {
      return RSVP.hash({
        ...
        calendarios: this.get('store').findAll('calendario', { reload: false}),
      });
    } else if (role === 'secretaria') {
      return RSVP.hash({
        pessoa: this.get('store').findRecord('pessoa', person.id, {}),
        instituicoes: this.get('store').query('instituicao', {
          instituicaoId: inst_id,
          include: 'areas, modulos, plataforma-anos.segmento, plataforma-turmas, sistemas, calendario', reload: false
        }),
        modulos: this.get('store').findAll('modulo',{
